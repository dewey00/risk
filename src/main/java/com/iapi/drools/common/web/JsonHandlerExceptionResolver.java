package com.iapi.drools.common.web;


import com.iapi.drools.common.exception.BusinessException;
...
public class JsonHandlerExceptionResolver implements HandlerExceptionResolver {
    ...
    private static Logger logger = LogManager.getLogger(JsonHandlerExceptionResolver.class);
    ...
    private void logException(Object handler, HttpServletRequest request, Exception exception, Map<String, String[]> parameterMap) {
        if (handler != null && HandlerMethod.class.isAssignableFrom(handler.getClass())) {
            try {
                ...
                String info;
                try {
                    info = request.getSession().getAttribute("AccountName").toString();
                } catch (Exception e) {
                    info = "iapi_guess";
                }
    ...
    private String msg(Exception e) {
        ...
        for (StackTraceElement element : elements) {
            ...
            if (className != null && className.startsWith("com.iapi")) {
                builder.append("\t");
